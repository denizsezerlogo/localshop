// İş kuralları katmanı: HTTP'den ve dilden bağımsızdır — hata durumunda
// hazır metin değil, mesaj ANAHTARI fırlatır (çeviri errorHandler'da yapılır).
...
export async function register({ name, email, password, role }) {
  ...
  if (existing) throw new ApiError(409, 'AUTH_EMAIL_TAKEN');
...
export async function login({ email, password }) {
  ...
  if (!valid) throw new ApiError(401, 'AUTH_BAD_CREDENTIALS');
