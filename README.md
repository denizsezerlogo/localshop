...
- **Token saklama (MVP trade-off):** JWT localStorage'da tutulur; XSS'e karşı üretimde httpOnly cookie tabanlı oturum tercih edilmelidir. Not: token cookie'de değil `Authorization` başlığında taşındığı için CSRF saldırı yüzeyi yoktur; cookie'ye geçilirse CSRF koruması (SameSite/token) eklenmelidir.
